...
var _ = Describe("Struct", func() {
	Context("correctness", func() {
		It("struct consts should be correct", func() {
			...
			Expect(unsafe.Sizeof(dbn.TcbboMsg{})).To(Equal(uintptr(dbn.TcbboMsg_Size)))
			...
			Expect(int((&dbn.TcbboMsg{}).RSize())).To(Equal(dbn.TcbboMsg_Size))
	...
	Context("Definition messages", func() {
		...
		It("should read v2 definition correctly", func() {
			...
			Expect(r0.UserDefinedInstrument).To(Equal(byte('N')))
			...
			Expect(r1.UserDefinedInstrument).To(Equal(byte('N')))
